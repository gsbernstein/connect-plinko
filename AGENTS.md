...
| `noteChipEarn` / `updateChipRateUI` / `chipRateFillPct` / `chipRateBarMax` / `chipRateMarkPeak` / `#chipRateVal` / `#chipRateFill` / `#chipRateLag` / `maxChipRate` | Rolling chips/s under the Chips HUD (5s window from `awardChips`; clears on Cash Out / reset) — meter hidden until Slide Rule Lv1; `#chipRateBar` reuses shop `.upgrade-progress` and sits flush on the bottom edge (no inset gap) — fill via `chipRateFillPct` (linear Lv1, log Lv2); `#chipRateLag` gold tick marks `chipRateMarkPeak` (lags behind while rate climbs, snaps on drops); `noteChipEarn` always records peak floored rate into `stats.maxChipRate` for Chip Flood (`a_chip_rate`) |
...
| Chips/s prior-peak tick | `#chipRateLag` `.chip-rate-lag` | `chipRateMarkPeak` in `updateChipRateUI` (gold tick at lagging peak while rate climbs) | — |
