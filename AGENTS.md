...
| `pinSplitActive` / `spawnSplitTwin` / `PIN_SPLIT_*` / `pendingManualDropX` / `flushPendingManualDrop` | Percussive Mitosis (`pinSplit`): ~10× slower drops; bifurcate on fresh peg hits (max gen 3); shrink in flight, full size in columns. Manual taps during the 2.4s cooldown queue into `pendingManualDropX` and fire via `flushPendingManualDrop` (tick / idleSpin) instead of being discarded. |
...
| Pin Split spawn / split | `autoDropIntervalMs` × `dropSpawnMult`, `manualDropCooldownMs` / `pendingManualDropX` / `flushPendingManualDrop` in `dropAt`, `canSplitBall` / `spawnSplitTwin` on fresh peg hits, size reset in `enterGridColumn` / `drawBall` (Percussive Mitosis) |
