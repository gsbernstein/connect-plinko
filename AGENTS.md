...
| `pushPopup` / `bakePopupSprite` / `separatePopupFromOthers` / `popupHalfExtents` / `trimPopups` / `popupFamily` / `popupWorth` / `MAX_POPUPS` / `expiresAt` | Score labels: baked offscreen sprite at spawn (`bakePopupSprite`; optional `subtext` second row for hand/King Me totals); spawn-time AABB nudge via `separatePopupFromOthers` (capped drift); per-frame `drawImage` + transform; draw order low→high `popupWorth`; value-scaled duration + `expiresAt` spacing (~160 ms); cap 120; over-cap drops by worth − duplicate index |
...
**Combo popups:** `comboPopupJuice(n)` / `spawnPopups` — COMBO ×N banner size/slam/glow scale with simultaneous hands; multi-hand clears fan per-hand labels (`CELL * 0.28`) and park COMBO lower (`CELL * 0.85`). Multi-hand clears call `spawnStaggeredChainPopups` (`CHAIN_POP_STAGGER_MS`, `CHAIN_POP_STACK_GAP` 36) so each heater step gets its own delayed CHAIN slam (`t0` in the future; draw skips `age < 0`). `trimPopups` uses `popupFamily` / `popupWorth` / `handPopupWorth`: duplicate labels in a family (PAIR, COMBO, CHAIN, …) drop after the best copy; lower hands and smaller COMBO/CHAIN × yield before rarer ones. Labels bake via `bakePopupSprite` at spawn (`subtext` = smaller chip total under hand / King Me titles); `separatePopupFromOthers` nudges new labels off active ones; the draw loop `drawImage`s sprites in low→high `popupWorth` order so stronger hands/combos land on top.
