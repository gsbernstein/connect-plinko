...
| `kingMeActive` / `kingStackLevel` / `kingKindWeight` / `resolveKingMerges` / `tryKingMergeOnce` / `kingMergeLabel` / `KING_ME_*` / `KING_CROWN_SLAM_*` | King Me: same-suit equal-stack kings that meet (4-dir) merge into `kingStack+1` with exponential payout (`KING_ME_MERGE_BASE * 2^tier`); popups `KING ME!` / `DOUBLE KING!` / `TRIPLE KING!` / …; `kingKindWeight` counts stacks in of-a-kind (`evaluateCards` → `N OF A KIND` past five); tiny golden crowns on the K via `drawKingCrowns` (one per tier); newest crown slams on with `crownSlamAt` / `KING_CROWN_SLAM_*` (`easeOutBack`); chains via `resolveKingMerges` after settles/clears |
| `handTextTier` / `handPopupPunctuation` / `TEXT_JUICE` | Score popup size/slam tier; of-a-kind punctuation is special-cased (five `!?!`, six+ `‽`×(n−5)) instead of huge-tier `!!!` |
...
| King Me | `kingMeActive` / `resolveKingMerges` on `finalizeDrop` / post-clear; same-suit equal `kingStack` neighbors merge (`tryKingMergeOnce`); payout `kingMergePayout`; `kingMergeLabel` / `kingKindWeight` / `drawKingCrowns` / `crownSlamAt`; Big Room merge sparks `spawnExplosion` |
