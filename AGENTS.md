...
| `purgeCardPayout` / `suitBombPickerMinimized` / `suitBombUiVisible` / `suitPurgePairPenalty` / `pickAutoSuitBombTarget` | Suit Purge: shop Hide/Show and bar tap minimize picker only (purges keep running); purge pays per card; auto-target ranks by pair-safe effective count but gates on raw cards in play (≥2) so Pin Split duplicate floods still cast; with Suit Exile, targets/buttons are suit families (`purgeFamilies`) and dual-glyph buttons show both allies |
| `suitCut` / `suitFamily` / `suitsInFamily` / `purgeFamilies` / `SUIT_ALIAS_STEPS` | Suit Exile: does **not** remove suits from the shoe; Lv1 ♦→♥, Lv2 also ♣→♠ for flush matching (`sliceSharesFlushSuit` / `flushHintColor` family color) and Suit Purge (one button/family, clears both printed suits). Cards keep their own felt/glyph. |
...
| Suit Purge UI | `#suitBombBar`, `suitBombUiVisible` / `suitBombPickerMinimized`, `updateSuitBombUI` / `repaintSuitBombGlyphs`; shop Hide/Show and bar tap minimize picker only; Suit Exile folds ally suits onto one `.pair-suit` button |
| Suit Exile (flush/purge allies) | `suitFamily` / `SUIT_ALIAS_STEPS` on `suitCut`; flush via `sliceSharesFlushSuit`; purge via `findSuitCells` / `castSuitBomb` family keys |
