...
| `noteChipEarn` / `updateChipRateUI` / `chipRateFillPct` / `chipRateTickPositions` / `chipRateValueAtPct` / `chipRateSnapSingleDigit` / `updateChipRateTicks` / `chipRateSmoothed` / `#chipRateRow` / `#chipRateVal` / `#chipRateFill` / `#chipRateTicks` / `maxChipRate` | Rolling chips/s under the Chips HUD (5s window from `awardChips`; clears on Cash Out / reset) — meter hidden until Slide Rule Lv1; `#chipRateBar` reuses shop `.upgrade-progress`; fill via `chipRateFillPct` (linear Lv1, log Lv2); `CHIP_RATE_TICK_SLOTS` fixed positions (20–80%) — linear values evenly spaced, log values log-spaced; labels snap to single-digit mantissa; labeled tick gets `.chip-rate-tick-label` above the mark; `/s` display eased via `chipRateSmoothed`; `?chipRateDebug=1` opens tuner panel |
