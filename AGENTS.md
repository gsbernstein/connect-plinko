...
| `bigRoomActive` / `castFireballsFrom` / `stepBurning` / `BIG_ROOM_*` | Big Room: explosions (incl. Storm pin shells) cast fireballs that ignite settled cards; burn ticks drip chips then ash out |
...
| Big Room fireballs | `bigRoomActive` / `spawnExplosion` → `castFireballsFrom` (Storm pin shells + hand/floor clears); `stepFireballs` / `stepBurning` / `ashOutStackEntry`; burn on stack entry `.burning` |
