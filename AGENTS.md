...
| `noteChipEarn` / `updateChipRateUI` / `chipRateFillPct` / `chipRateMarkAt` / `chipRateLastMax` / `#chipRateVal` / `#chipRateFill` / `#chipRateLag` / `maxChipRate` | Rolling chips/s under the Chips HUD (5s window from `awardChips`; clears on Cash Out / reset) — meter hidden until Slide Rule Lv1; `#chipRateBar` reuses shop `.upgrade-progress` and sits flush on the bottom edge (no inset gap) — fill via `chipRateFillPct` (linear Lv1, log Lv2); `#chipRateLag` gold tick marks prior settled `maxChipRate` when the bar scale rises (`chipRateMarkAt` eases up; a new peak mid-ease re-anchors to `chipRateLastMax`); `noteChipEarn` bumps `stats.maxChipRate` for Chip Flood (`a_chip_rate`) |
...
| Chips/s prior-peak tick | `#chipRateLag` `.chip-rate-lag` | `chipRateMarkAt` / `chipRateLastMax` in `updateChipRateUI` (tick on `maxChipRate` rise; re-anchors to prior settled peak if scale bumps again mid-ease) | — |
