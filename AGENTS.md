...
| `PRESTIGE_DEFS` | VIP perks bought with comps after Cash Out — array order = VIP tab order, sorted by Lv1 `baseCost` ascending (ties by name). Skill-bonus perks (`quickDeal` → Auto Dealer, `pinPrivilege` → Pin Tip, `feltWax` → Felt Grease) use `prestigeBonusLevels`. |
