...
| Symbol | Purpose |
|--------|---------|
| `UPGRADE_DEFS` | Table shop upgrades (Auto Dealer, Big Blind, Kickbacks / `houseCut`, Cashier’s Cage / `compCage`, …) — order = shop order |
| `houseCut` / `pinHitPayout` / `bumperHitPayout` / `floorClearPayout` | Kickbacks (unlockAt 5): one level raises peg, bumper, and floor-clear chip payouts; replaces Pin Tip / Rail Tax / Sweep Stakes (`loadSave` v27 migrates max of the three). They Know Me `pinPrivilege` effect adds bonus levels via `prestigeBonusLevels`. |
| `SHOP_RECOMMEND_PRIORITY` / `VIP_RECOMMEND_PRIORITY` / `recommendedShopUpgradeId` / `recommendedVipPerkId` / `.upgrade-rec` / `.upgrade-cost-row` | Gold ★ after the price on the next guided buy: shop Auto Dealer→6 → Wild Card→1 → Kickbacks→1 → High Cut→1 → Suit Alliance→2; VIP Quest Desk→1 → Quest Ink→4. First incomplete unlocked step wins (`fillUpgradeButton` `recommended`; `.upgrade-rec.is-on` via `visibility`) |
...
| `PRESTIGE_DEFS` | VIP perks bought with comps after Cash Out (`theyKnowMe`, `rackDiscount`, `compCard`, `blindSpread`, `questInk`, `aceSleeve`, `connect4`, `dealMeIn`, `cardCounting`, `pinSplit`, `stormBeaches`, `siegeWeapons`, `kingMe`, `bigRoom`, `controlBooth`, …). Skill-bonus levels for Hot Streak / Auto Dealer / Kickbacks / Felt Grease come from `theyKnowMe` via `prestigeBonusLevels`. |
| `theyKnowMe` / `theyKnowMeEffectLevel` / `theyKnowMeCarryoverPercent` / `noteRunUpgradePeak` / `runPeakUpgradeId` | Next-table VIP track (replaces Seed Stack, House Edge, Quick Deal, Pin Privilege, Felt Wax): starting chips from Lv 1; Hot Streak bonus Lv 4+; Auto Dealer Lv 8+; Kickbacks Lv 12+; Felt Grease Lv 16+; upgrade carryover Lv 20+ (15–50% of peak purchased level on Cash Out). Rack Discount / Comp Card / Open Blind / Quest Ink stay separate. `loadSave` v27 migrates legacy perk levels. |
...
| New VIP perk | `PRESTIGE_DEFS`, `prestigeEffectText`, buy UI; skill bonuses via `theyKnowMe` / `prestigeBonusLevels`; hand cheats via `evaluateHandSlice` / `handPayoutMultiplier` / `flushMinLength` (Connect 4); Deal Me In via `dealMeInActive` / `tryJoinResolve` on late `finalizeDrop`; Pin Split via `pinSplitActive` / peg hit in `stepBall` |
