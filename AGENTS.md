...
| `handTextTier` / `handPopupPunctuation` / `handPopupDuration` / `popupDurationForValue` / `pushPopup` / `TEXT_JUICE` | Score popup size/slam tier; of-a-kind punctuation (five `!?!`, six+ `‽`×(n−5)); duration scales with payout; `pushPopup` sets `expiresAt` and nudges end times apart (~160 ms) so bursts don’t clear in one frame |
