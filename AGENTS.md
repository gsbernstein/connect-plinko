...
| King Me | `kingMeActive` / `resolveKingMerges` on `finalizeDrop` / post-clear; same-suit equal `kingStack` neighbors merge (`tryKingMergeOnce`); payout `kingMergePayout`; `kingMergeLabel` / `kingKindWeight` / `drawKingCrowns`; Big Room merge sparks `spawnExplosion` |
