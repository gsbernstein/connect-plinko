...
| `bigRoomActive` / `igniteNearbyFromBlast` / `handBurnMultiplier` / `stepBurning` / `maybeFlamingKingIgnite` / `BIG_ROOM_*` | Big Room: explosions instantly ignite settled cards in blast radius; burn ticks drip chips then ash out; hands with burning cards get +50%/card via `handBurnMultiplier`; crowned King Me leaps go aflame and light nearby cards |
...
| Big Room blast ignite | `bigRoomActive` / `spawnExplosion` → `igniteNearbyFromBlast` (instant within `BIG_ROOM_BLAST_IGNITE_R`); `handBurnMultiplier` / `countBurningInCells` in `findPokerHands`; `stepBurning` / `ashOutStackEntry` (`noIgnite`); flaming King Me via `launchKingJumper` → `maybeFlamingKingIgnite` |
