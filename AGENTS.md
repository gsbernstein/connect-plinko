...
| `drawResolveHandLines` / `cellsFormLine` | Paint-only flash connectors on `plateCtx` before seat holes are punched (so strokes remain only on plastic gaps; no labels; skips Suit Purge scatter). Does not touch score/clear timing; near-miss guides stay off while resolving |
