...
| `kingMeActive` / `kingStackLevel` / `kingKindWeight` / `resolveKingMerges` / `tryKingMergeOnce` / `kingMergeLabel` / `KING_ME_*` | King Me: same-suit equal-stack kings that meet (4-dir) merge into `kingStack+1` with exponential payout (`KING_ME_MERGE_BASE * 2^tier`); popups `KING ME!` / `DOUBLE KING!` / `TRIPLE KING!` / …; `kingKindWeight` counts stacks in of-a-kind (`evaluateCards` → `N OF A KIND` past five); tiny golden crowns perched on the K glyph via `drawKingCrowns` (one per tier); chains via `resolveKingMerges` after settles/clears |
