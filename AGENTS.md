...
| `drawResolveHandLines` / `cellsFormLine` | During flash resolve, suit-colored gap segments between seats for each collinear scoring group (inset past `HOLE_R`, no labels; skips Suit Purge scatter); near-miss guides stay off while resolving |
