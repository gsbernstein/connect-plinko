...
| `pushPopup` / `bakePopupSprite` / `trimPopups` / `popupFamily` / `popupWorth` / `MAX_POPUPS` / `expiresAt` | Score labels: baked offscreen sprite at spawn (`bakePopupSprite`); per-frame `drawImage` + transform only (no strokeText/shadowBlur); value-scaled duration + `expiresAt` spacing (~160 ms); cap 72; over-cap drops by worth − duplicate index |
