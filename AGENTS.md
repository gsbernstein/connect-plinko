...
| `noteChipEarn` / `updateChipRateUI` / `chipRateFillPct` / `chipRateMantissaSequence` / `chipRateLinearSequence` / `chipRateMilestones` / `updateChipRateTicks` / `chipRateSmoothed` / `#chipRateRow` / `#chipRateVal` / `#chipRateFill` / `#chipRateTicks` / `maxChipRate` | Rolling chips/s under the Chips HUD (5s window from `awardChips`; clears on Cash Out / reset) — meter hidden until Slide Rule Lv1; `#chipRateBar` reuses shop `.upgrade-progress`; fill via `chipRateFillPct` (linear Lv1, log Lv2); milestones are a contiguous value ladder (linear: even step grid; log: 1–9 per decade) — show up to `CHIP_RATE_TICK_SLOTS` visible ticks, drop from the left only; labeled tick gets `.chip-rate-tick-label`; `/s` eased via `chipRateSmoothed`; `?chipRateDebug=1` tuner panel |
