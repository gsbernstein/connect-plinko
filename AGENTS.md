...
| `pushPopup` / `trimPopups` / `popupFamily` / `popupWorth` / `MAX_POPUPS` / `expiresAt` | Score labels: value-scaled duration + `expiresAt` spacing (~160 ms); cap 40; over-cap drops by worth − duplicate index (same hand/COMBO/CHAIN family stacks yield first; lower hands & smaller COMBO/CHAIN × before rarer clears) |
