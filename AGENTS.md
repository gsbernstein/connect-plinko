...
| `kingMeActive` / `kingStackLevel` / `kingKindWeight` / `resolveKingMerges` / `tryKingMergeOnce` / `tryMergeKingBalls` / `tryMergeKingBallWithSettled` / `kingMergeLabel` / `KING_ME_*` / `KING_CROWN_SLAM_*` | King Me: same-suit equal-stack kings merge on contact anywhere — settled 4-dir (`tryKingMergeOnce`), falling↔falling (`tryMergeKingBalls` in `collideBalls`), or falling↔settled (`tryMergeKingBallWithSettled`); payout `KING_ME_MERGE_BASE * 2^tier`; popups `KING ME!` / `DOUBLE KING!` / …; `kingKindWeight` for of-a-kind; crowns on the K via `drawKingCrowns` + `crownSlamAt` slam; victim fallers flagged `kingMerged` then swept |
...
| `pushPopup` / `trimPopups` / `MAX_POPUPS` / `expiresAt` | Score labels: value-scaled duration + `expiresAt` spacing (~160 ms); cap 40; over-cap drops ephemeral hit pops, then surplus `chain` CHAIN banners (keeps highest × / newest one for full duration), before hand/COMBO celebrations |
...
**Combo popups:** `comboPopupJuice(n)` / `spawnPopups` — COMBO ×N banner size/slam/glow scale with simultaneous hands. Multi-hand clears call `spawnStaggeredChainPopups` (`CHAIN_POP_STAGGER_MS`) so each heater step gets its own delayed CHAIN slam (`t0` in the future; draw skips `age < 0`). CHAIN banners set `chain` / `chainMult`; `trimPopups` drops surplus CHAIN before hand/COMBO labels but keeps one (highest × / newest) for its full duration.
