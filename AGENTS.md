...
| `pushPopup` / `bakePopupSprite` / `separatePopupFromOthers` / `relaxPopupPositions` / `popupNeedsSeparation` / `popupHalfExtents` / `trimPopups` / `popupFamily` / `popupWorth` / `MAX_POPUPS` / `expiresAt` | Score labels: baked offscreen sprite at spawn (`bakePopupSprite`; optional `subtext` second row for hand/King Me totals); hand/COMBO spawn-time AABB nudge via `separatePopupFromOthers` (drift from `spawnX`/`spawnY`; CHAIN / peg hits skipped); multi-hand bursts call `relaxPopupPositions` so low-worth hand/COMBO yields; per-frame `drawImage` + transform; draw order low→high `popupWorth`; value-scaled duration + `expiresAt` spacing (~160 ms); cap 120; over-cap drops by worth − duplicate index |
