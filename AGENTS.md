...
| `pushPopup` / `trimPopups` / `MAX_POPUPS` / `expiresAt` | Score labels: value-scaled duration + `expiresAt` spacing (~160 ms); cap 96; over-cap drops ephemeral hit pops first (not FIFO of whole prior clears) |
