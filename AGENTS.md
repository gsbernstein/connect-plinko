...
| `bigRoomActive` / `igniteNearbyFromBlast` / `markBallLit` / `handBurnMultiplier` / `stepBurning` / `maybeFlamingKingIgnite` / `BIG_ROOM_*` | Big Room: explosions instantly ignite settled cards (`BIG_ROOM_BLAST_IGNITE_R`) and in-flight chips (`BIG_ROOM_FLIGHT_IGNITE_R` = Storm shove radius); `pendingBurn` transfers to the seat in `finalizeDrop`; burn ticks drip chips then ash out; hands with burning cards get +50%/card via `handBurnMultiplier`; crowned King Me leaps go aflame and light nearby cards |
