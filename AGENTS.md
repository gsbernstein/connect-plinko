...
| `noteChipEarn` / `updateChipRateUI` / `chipRateFillPct` / `chipRateMilestones` / `updateChipRateTicks` / `chipRateSmoothed` / `#chipRateRow` / `#chipRateVal` / `#chipRateFill` / `#chipRateTicks` / `maxChipRate` | Rolling chips/s under the Chips HUD (5s window from `awardChips`; clears on Cash Out / reset) — meter hidden until Slide Rule Lv1; `#chipRateBar` reuses shop `.upgrade-progress`; fill via `chipRateFillPct` (linear Lv1, log Lv2); `#chipRateTicks` milestone rules slide on rescale (`transition:left`); labeled tick gets `.chip-rate-tick-label` above the mark; `/s` display eased via `chipRateSmoothed`; `?chipRateDebug=1` opens tuner panel |
