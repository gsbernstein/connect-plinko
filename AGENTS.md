...
| `pushPopup` / `bakePopupSprite` / `separatePopupFromOthers` / `relaxPopupPositions` / `popupNeedsSeparation` / `resolveRegionBounds` / `comboPopupPosition` / `popupHalfExtents` / `trimPopups` / `popupFamily` / `popupWorth` / `MAX_POPUPS` / `expiresAt` | Score labels: baked offscreen sprite at spawn (`bakePopupSprite`; optional `subtext` second row for hand/King Me totals); hand/COMBO spawn-time AABB nudge via `separatePopupFromOthers` (hands drift-capped from `spawnX`/`spawnY`; COMBO clamps to `resolveRegionBounds` of the clear; CHAIN / peg hits skipped); multi-hand bursts call `relaxPopupPositions` so low-worth hand/COMBO yields; per-frame `drawImage` + transform; draw order low→high `popupWorth`; value-scaled duration + `expiresAt` spacing (~160 ms); cap 120; over-cap drops by worth − duplicate index |
...
| ~6830 | `pushPopup` / `bakePopupSprite` / `clampPopupToBoard` / `separatePopupFromOthers` / `#popupLayer` — score labels bake once, hang up to `POPUP_OVERHANG` past the felt; hand/COMBO nudge off neighbors at spawn (CHAIN stacks untouched) |
...
**Combo popups:** `comboPopupJuice(n)` / `spawnPopups` — COMBO ×N banner size/slam/glow scale with simultaneous hands; multi-hand clears fan per-hand labels (`CELL * 0.4`), place COMBO via `comboPopupPosition` / `resolveRegionBounds` (clearest spot in the cleared-cell bbox), then `relaxPopupPositions` so weaker hand/COMBO labels yield — COMBO may roam anywhere in that region (not a fixed drift cap). Multi-hand clears call `spawnStaggeredChainPopups` (`CHAIN_POP_STAGGER_MS`, stack gap 12) so each heater step gets its own delayed CHAIN slam in the peg zone (`t0` in the future; draw skips `age < 0`) — CHAIN keeps its intentional tight stack and is excluded from separation. `trimPopups` uses `popupFamily` / `popupWorth` / `handPopupWorth`: duplicate labels in a family (PAIR, COMBO, CHAIN, …) drop after the best copy; lower hands and smaller COMBO/CHAIN × yield before rarer ones. Labels bake via `bakePopupSprite` at spawn (`subtext` = smaller chip total under hand / King Me titles); `separatePopupFromOthers` only nudges hand/COMBO (`popupNeedsSeparation`); the draw loop `drawImage`s sprites in low→high `popupWorth` order so stronger hands/combos land on top.
