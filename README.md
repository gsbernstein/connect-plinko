...
4. **VIP / prestige** — from run level 8, **Cash Out** banks your level comps for permanent perks. **Hold Deal** is a VIP perk (hold-to-drop, always available). **Table Tilt** steers peg falls from device motion (toggleable; motion permission only when you turn it on). **Quick Deal** / **Pin Privilege** add bonus Auto Dealer / Pin Tip levels without raising those upgrades’ buy costs (gold on the progress bar; still capped at the skill max). Also: House Edge, Seed Stack, Rack Discount, Comp Card, Quest Ink, Pit Boss. Hard reset wipes VIP and achievements too.
