...
Progression layers:
1. **Chips** — spend on table upgrades (Auto Dealer, Multi Deal, Big Blind, Pin Tip, …). Auto Dealer is cheap through Lv 6 then pricier; **Multi Deal** adds a chance for extra cards per auto tick (caps at 150%; above 100% can deal multiple). Hot Streak caps at Lv 10. **Big Blind** goes to Lv 60; **Premium Cut** to Lv 25; **Felt Grease** to Lv 30 (late levels keep speeding falls with softer gains). **Cashier’s Cage** (unlocks at run Lv 10) spends chips to bank Cash Out comps — a late-game sink that feeds VIP instead of more chip income. **Short Fuse** speeds hand clears but is toggleable when you want more time to stack combos or Card Counting pairs. **Suit Purge** pays per card; the shop toggle or bar tap can hide the suit picker without stopping purges. The HUD shows your balance and a live chips/s rate. Many upgrades stay hidden until unlocked.
