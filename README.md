...
Progression layers:
1. **Chips** — spend on table upgrades (Auto Dealer, Multi Deal, Big Blind, Pin Tip, …). Auto Dealer is cheap through Lv 6 then pricier; **Multi Deal** adds a chance for extra cards per auto tick (above 100% can deal multiple). Hot Streak caps at Lv 10. **Short Fuse** speeds hand clears but is toggleable when you want more time to stack combos or Card Counting pairs. **Suit Purge** pays per card; the shop toggle or bar tap can hide the suit picker without stopping purges. The HUD shows your balance and a live chips/s rate. Many upgrades stay hidden until unlocked.
...
4. **VIP / prestige** — from run level 8, **Cash Out** collects your Cash Out comps into the VIP balance for permanent perks. **Table Tilt** steers peg falls from device motion (toggleable; motion permission only when you turn it on). **Quick Deal** / **Pin Privilege** / **Felt Wax** add bonus Auto Dealer / Pin Tip / Felt Grease levels (gold on the progress bar; still capped at the skill max) and raise those upgrades’ buy costs like normal levels. **House Edge** adds permanent Hot Streak levels the same way. **Open Blind** extends Big Blind’s chip multiplier to more payout types each level (pin tips → rail tax → sweep stakes → quests & achievements → level bonuses). **Ace Up Your Sleeve** (expensive) scores each line as the best hand from its cards plus a hidden Ace. **Connect 4** (expensive) lets flushes score with only 4 suited cards. **Card Counting** (eight levels) adds +8% hand payout per duplicate on the board per level; the ninth tier (very expensive) multiplies your level payout mult by the product of every paired card on the board. **Percussive Mitosis** (expensive, toggleable) slows drops ~10× but cards bifurcate on pins — shrinking each bounce, then full size in the columns. **Storm the Beaches** (expensive, toggleable) speeds Auto Dealer ~3× and cards can randomly explode on pins, but survivors that reach the columns earn chips. **I Didn't Ask How Big the Room Was** (expensive, toggleable) makes explosions light nearby cards on fire — including chips still falling through the pins — burning cards drip chips until they ash out, hands with lit cards pay a bonus, and crowned kings leap aflame lighting neighbors. **Siege Weapons** (up to 3 levels, toggleable) gives some cards spikes that smash bumpers for Rail Tax — Lv 1 clears a column, Lv 2 clears every column on that bumper, Lv 3 slams the landing column for a combo payout. **King Me** (expensive, toggleable) crowns kings that reach the bottom row — they stack a second king, leap diagonally over cards for big chips, then explode after a few jumps. Also: Seed Stack, Rack Discount, Comp Card, Quest Ink, Card Sense, Auto Purge, Pit Boss. Hard reset wipes VIP and achievements too.
...
Saves in the browser — including your Hot Streak chain, so a refresh or update doesn’t reset the heater. Near-miss guides show when you’re close; a full board still triggers the automatic floor clear. **Settings** lists What’s new; a red badge appears when there are updates since your last save viewed that panel, and those entries are highlighted as New. If a new Poker build ships while a tab is still open, a **Refresh** banner can appear after checking `poker/version.json`.
