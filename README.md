...
4. **VIP / prestige** — from run level 8, **Cash Out** banks your level comps for permanent perks. **Hold Deal** is a VIP perk (hold-to-drop, always available). **Table Tilt** steers peg falls from device motion (toggleable; motion permission only when you turn it on). **Quick Deal** / **Pin Privilege** / **Felt Wax** add bonus Auto Dealer / Pin Tip / Felt Grease levels without raising those upgrades’ buy costs (gold on the progress bar; still capped at the skill max). **House Edge** adds permanent Hot Streak levels the same way. **Open Blind** extends Big Blind’s chip multiplier to more payout types each level (pin tips → rail tax → sweep stakes → quests & achievements → level bonuses). Also: Seed Stack, Rack Discount, Comp Card, Quest Ink, Card Sense, Auto Purge, Pit Boss. Hard reset wipes VIP and achievements too.
