...
2. **Run level** — rises automatically from chips *earned* this table. Levels unlock shop items and grant rewards (VIP comps banked for Cash Out, scaling chip bonuses at milestones, and late-game multiplier boosts). Click the level pill to see what you’ve earned and what’s next. Resets when you Cash Out.
3. **Quests & achievements** — **Quests** stay locked until run Lv 2, then additional slots unlock at Lv 20, 45, 75, and 110 (max 5). Claiming a quest rolls a replacement. Cash Out clears the quest board (achievements stay). Achievement rewards often grant **instant comps** (VIP comps right away); some quests bank VIP comps instead. Rarity starts at Common; VIP **Quest Ink** unlocks Uncommon → Legendary.
4. **VIP / prestige** — from run level 8, **Cash Out** collects your banked VIP comps for permanent perks. **Table Tilt** steers peg falls from device motion (toggleable; motion permission only when you turn it on). **Quick Deal** / **Pin Privilege** / **Felt Wax** add bonus Auto Dealer / Pin Tip / Felt Grease levels (gold on the progress bar; still capped at the skill max) and raise those upgrades’ buy costs like normal levels. **House Edge** adds permanent Hot Streak levels the same way. **Open Blind** extends Big Blind’s chip multiplier to more payout types each level (pin tips → rail tax → sweep stakes → quests & achievements → level bonuses). **Ace Up Your Sleeve** (expensive) scores each line as the best hand from its cards plus a hidden Ace. **Card Counting** boosts hand payouts for each duplicate card (same rank and suit) on the table. Also: Seed Stack, Rack Discount, Comp Card, Quest Ink, Card Sense, Auto Purge, Pit Boss. Hard reset wipes VIP and achievements too.
